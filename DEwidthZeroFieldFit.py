...
######################################
######################################
def de(freq,signal,zfArray): # determines D, E, and width from zero-field data
...
##freq, signal = np.loadtxt('%s-%s.txt'%(sample,startIndex),delimiter=', ',skiprows=1,\
##                          unpack=True)
##for i in range((startIndex+1),stopIndex,1):
##    freq, signaltmp = np.loadtxt('%s-%s.txt'%(sample,i),delimiter=', ',skiprows=1,\
##                          unpack=True)
...
##np.savetxt('%s-sum.txt'%sample,NVsumArray,delimiter=', ',\
##           header='freq, data', comments='')
...
##                          unpack=True)
##freq1,signal1 = np.loadtxt('NV-after-sum.txt',delimiter=', ',skiprows=1,\
##                          unpack=True)
##signal = signal/np.amax(signal)
##signal1 = signal1/np.amax(signal1)
##zfArray = np.array([2.871e9,3e6,2.2e6,0,\
##                0,0,0,0])
...
##    np.loadtxt('NV-Co-fit.txt',delimiter=', ',\
